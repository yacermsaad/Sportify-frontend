      ...
      <div className={`md:flex items-center space-x-4 ${isMenuOpen ? 'block' : 'hidden'} md:block`}>
        ...
        {!userInitial && (
          <>
            ...
            <a className="block md:inline-block mt-2 md:mt-0 text-green-500 hover:text-green-700 cursor-pointer text-bold text-md text-center" style={{ color: "#04cfb4" }} onClick={openModalLogin}>{t('Login')}</a>
            <Login isOpen={isModalOpenLogin} setOpen={closeModal} />
            <span className="text-gray-400 max-md:hidden">|</span>
            <a className="block md:inline-block mt-2 md:mt-0 text-white font-bold py-2 px-4 bg-gradient-to-r from-green-400 to-green-600 hover:from-green-500 hover:to-green-700 rounded-full text-bold text-md cursor-pointer" style={{ background: "#04cfb4" }} onClick={openModalCreate}>{t('Register')}</a>
            ...
            <div className="relative inline-block text-left rounded-lg pl-6 py-1 shadow-sm">
              ...
              <label htmlFor="dropdownAvatarNameToggle" className="flex items-center text-sm pe-1 text-black font-semibold rounded-full cursor-pointer hover:text-blue-600 dark:hover:text-blue-500 md:me-0 focus:ring-4 focus:ring-gray-100 ">
                {image_user === 'null' ?
                  ...
                  <div className="flex items-center justify-center w-8 h-8 text-white rounded-full mr-4">
                    <img src={`http://localhost:8000/storage/${image_user}`} alt="User Avatar" />
                  </div>
                }
              ...
              <div id="dropdownAvatarName" className="absolute right-0 hidden bg-white divide-y divide-gray-300 rounded-lg shadow w-44 peer-checked:block mt-2 z-[999]">
                ...
                <ul className="py-2 text-sm text-gray-900">
                  <li>
                    <a href={localStorage.getItem('coache') === '0' ? '/becom_coache' : '/pro'} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-green-500 dark:hover:text-white">
                      {localStorage.getItem('coache') === '0' ? t('Become a Coach') : t('Switch to Coach')}
                    </a>
                  </li>
                  <li>
                    <a onClick={() => { setisModalOpenLike(true) }} className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-green-500 dark:hover:text-white cursor-pointer">{t('Update your picture')}</a>
                    <Profile_popup isOpen={isModalOpenLike} setOpen={closeModal} getdata={props.getdata} />
                  </li>
                  <li>
                    <a href="/settings" className="block px-4 py-2 hover:bg-gray-100 dark:hover:bg-green-500 dark:hover:text-white">{t('Settings')}</a>
                  </li>
                </ul>
                <div className="py-2 cursor-pointer" onClick={handleLogout}>
                  <a className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-100 dark:hover:bg-green-500 dark:text-gray-900 dark:hover:text-white">{t('Sign out')}</a>
                </div>
              </div>
            </div>
          </div>
